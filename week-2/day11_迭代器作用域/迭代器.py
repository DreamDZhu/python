...
# 可迭代对象与迭代器的对比
'''
可迭代对象是一个操作方法比较多，数据增删改查直观，数据存储相对少的一个数据集
应用： 当侧重于数据灵活处理，并且内存空间足够，将数据集设置为可迭代对象是明确的选择。

迭代器
非常节省内存，记录取值位置，可以直接通过循环+next方法取值，但是不直观，适合操作比较单一的数据集
应用： 当数据量过大，大到一次性读入可能撑爆内存，将数据集设置成迭代器是一个非常好的选择。
